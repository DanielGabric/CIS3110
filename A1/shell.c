...
/*Just some constants for initializing strings*/
#define INITIAL 100
#define ARGUMENT_SIZE 100


/*Holds all types of arguments*/
typedef struct 
{
    char ** arguments;
    int size;
...
/*Checks if command has pipe in it and then splits in two*/
void hasPipe(Input * in)
{
...
/*Single pipe, not multiple piping*/
void piping(Input * in)
{
...
void execute(Input* in)
{
    ...
    if(childPid == 0)
    {
        if(in->isInput)
        {
            if(!freopen(in->input_file,"r",stdin))
            {
        ...
        if(in->isOutput)
        {
            if(!freopen(in->output_file,"w+",stdout))
            {
                printf("Error, file is invalid!\n");
            }
        }
...
/*Multiplies all numbered arguments*/
void multiply(Input * list)
{
    int i;
    int tot = 1;
